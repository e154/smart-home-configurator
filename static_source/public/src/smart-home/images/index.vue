    ...
    <el-row>
      <el-col>
        <el-button
          ...
          <i class="el-icon-plus"/> {{ $t('images.addNew') }}
        </el-button>
      </el-col>
    </el-row>
    <el-row>
...
export default class extends Vue {
  ...
  private async getList() {
    ...
    const {data} = await api.v1.imageServiceGetImageList({
  ...
  private goto(area: ApiImage) {
    router.push({path: `/images/edit/${area.id}`});
  }

  private add() {
    router.push({path: `/images/new`});
  }
}
</script>
